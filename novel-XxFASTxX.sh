...
PARALLEL_JOBS=$(nproc)  # デフォルトの並列数はCPUコア数
...
MISC OPTIONS:
  ...
  -P, --parallel <NUM>     Set the number of parallel jobs (default: CPU cores)

SOURCE OPTIONS:
  -m, --save-my-bookmarks  Save all my bookmarked novels
                             Lazy mode: ID check (enable it by -c)
...
EXAMPLES:
	...
	$1 --with-inline-images --with-inline-files -F ID -P 8
EOF
}
...
download_cover_image() {
    ...
    mkdir -p -- "$(dirname -- "${2}")"
    curl -s -o "${2}" "${1}" || errquit "cover image download failed"
    return 0
}
...
download_inline_images() {
    ...
    for i in $(grep -o -E '\[(pixiv|uploaded)image:[0-9-]+\]' <<< "$1"); do
        ...
        if [ -z "$url" ]; then
            ...
            curl -s -o "${DIR_PREFIX}/illusts/${illust}.${ext}" "${url}" || errquit "inline image(s) download failed"
        fi
...
download_fanbox_inline_stuff() {
    ...
    mkdir -p -- "${prefix}/${type}/"
    curl -s -o "${prefix}/${type}/${post_id}-${id}${name}.${ext}" "${url}" || errquit "fanbox inline $type download failed"
}
...
download_novel() {
    ...
    # 既存ファイルのチェックをIDベースで行う
    local dir_part=$(dirname -- "$filename")
    local file_pattern="${dir_part}/${meta[id]}-*"
    shopt -s nullglob
    existing_files=($file_pattern)
    shopt -u nullglob
...
save_bookmarks() {
    ...
    while true ; do
        ...
        for i in $(seq 0 $(( $works_length - 1 ))) ; do
            ...
            download_novel "/bookmarks/${user}${suffix}" novel_meta textcount ""
        done
...
save_author() {
    ...
    while true ; do
        ...
        for i in $(seq 0 $(( $works_length - 1 ))) ; do
            ...
            download_novel "/by-author" novel_meta textcount ""
        done
...
[ "${#novels[@]}" = '0' ] || {
    ...
    printf "%s\n" "${novels[@]}" | xargs -n 1 -P "$PARALLEL_JOBS" -I {} bash -c 'save_id "{}"'
}

[ "${#authors[@]}" = '0' ] || {
    echo "[info] saving novels by author..."
    printf "%s\n" "${authors[@]}" | xargs -n 1 -P "$PARALLEL_JOBS" -I {} bash -c 'save_author "{}"'
}
...
[ "${#fanbox[@]}" = '0' ] || {
    ...
    printf "%s\n" "${fanbox[@]}" | xargs -n 1 -P "$PARALLEL_JOBS" -I {} bash -c 'save_id "{}" fanbox_post'
}

[ "${#fanbox_authors[@]}" = '0' ] || {
    echo "[warning] pixivFANBOX support is early experimental, there are many things not completed. If you meet a problem, please submit an issue"
    echo "[info] saving posts by fanbox user..."
    printf "%s\n" "${fanbox_authors[@]}" | xargs -n 1 -P "$PARALLEL_JOBS" -I {} bash -c 'save_fanbox_author "{}"'
}